...
function addMarkersFromFirebase(map) {
    ...
    onValue(binsRef, (snapshot) => {
        ...
        for (const binUID in binsData) {
            ...
            if (binData && binData.Location && binData.Location.Latitude && binData.Location.Longitude && binData.TotalQuota >= 4) {
                // Check if marker already exists
                if (!markers[binUID]) {
                    // Create a marker for each garbage bin
                    const marker = new google.maps.Marker({
                        position: { lat: binData.Location.Latitude, lng: binData.Location.Longitude },
                        map: map,
                        title: `GCN: ${binUID}`,
                        icon: defaultIcon
                    });
                    ...
                    const updateInfoWindow = () => {
                        contentDiv.innerHTML = `
                            <div>
                                <p>${binUID}</p>
                                <p>District: ${binData.Users[Object.keys(binData.Users)[0]].district.replace("District ", "")}</p>
                                <p>Barangay: ${binData.Users[Object.keys(binData.Users)[0]].barangay.replace("Brgy ", "")}</p>
                    ...
                    const binRef = ref(db, `GarbageBinControlNumber/${binUID}/TotalQuota`);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function filterMarkers(binsData, selectedDistrict, selectedBarangay) {
        for (const binUID in markers) {
            const binData = binsData[binUID];
            if (binData && binData.Users && binData.Users[Object.keys(binData.Users)[0]]) {
